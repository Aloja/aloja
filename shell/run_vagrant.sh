...
usage() {
  echo "Usage: $0 [-n net <IB|ETH>] [-d disk <HDD>] [-b benchmark <_min|_10>] [-r replicaton <positive int>]\
  ...
  echo "example: $0 -n IB -d HDD -r 1 -m 12 -i 10 -p 3 -b _min -I 4096 -l wordcount -c 1"
  exit 1;
}
...
SOURCE_DIR="$BLOBS_DIR/aplic"
HADOOP_VERSION="hadoop-1.0.3"
H_DIR="$HDD/aplic/$HADOOP_VERSION" #execution dir
HIB_DIR="$BLOBS_DIR/aplic/HiBench${BENCH}/"

#Location of prepared inputs
SAVE_LOCATION="$BLOBS_DIR/HiBench_prepare/"
...
JOB_PATH="$BLOBS_DIR/jobs/$JOB_NAME"
...
zabbix_sender(){
  :
  #echo "minerva-1001 $1" | $alojaHomePrefix/npoggi/aplic/zabbix/bin/zabbix_sender -c $alojaHomePrefix/npoggi/aplic/zabbix/conf/zabbix_agentd.conf -T -i - 2>&1 > /dev/null
  #>> $LOG_PATH
}
...
EOF
  ...
  $DSH_MASTER "mkdir -p $JOB_PATH/conf_$host1" 2>&1 |tee -a $LOG_PATH
