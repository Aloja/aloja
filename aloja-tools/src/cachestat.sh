...
#             Uses Linux ftrace.
#
# From: https://github.com/brendangregg/perf-tools with some modifications
#
...
interval=1; opt_timestamp=0; opt_hostname=; opt_debug=0
...
### process options
while getopts Dnht opt
do
	case $opt in
	D)	opt_debug=1 ;;
	n)	opt_hostname="$(hostname)" ;;
	t)	opt_timestamp=1 ;;
	h|?)	usage ;;
	esac
done
...
(( opt_timestamp )) || echo "Counting cache functions... Output every $interval seconds."
...
[ $opt_hostname  ] && printf "%-16s " HOSTNAME
(( opt_timestamp )) && printf "%-10s " TIMESTAMP
...
while (( !quit && (!opt_duration || secs < duration) )); do
	...
	[ $opt_hostname ] && printf "%-16s " "$opt_hostname"
	(( opt_timestamp )) && printf "%10(%s)T " -1
